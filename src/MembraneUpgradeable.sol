...
contract MembraneUpgradeable is Initializable {
    ...
    function __Membrane_init(address _owner) internal onlyInitializing {
        __Membrane_init_unchained(_owner);
    }

    function __Membrane_init_unchained(address _owner) internal onlyInitializing {
        owner = _owner;
    }
        ...
        external
    {
        ...
        for (uint256 i = 0; i < _members.length; i++) {
            if (_members[i] == _memberaddress) {
               _members[i] = _members[_members.length - 1]; //swap position with last member
               break;
            }
        }
